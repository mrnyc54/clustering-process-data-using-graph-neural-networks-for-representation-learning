...
def gen_hyperparams(engine, length, seed=None, whitelist=None, att_chance=None, extended_mode=0):
    # from hyperparam combi: gargantuan_detestable_statement
    vector_dimensions = [2, 3, 4, 8, 16, 32, 64, 128, 256]
    hyperparams_name = gen_df(length*len(vector_dimensions), seed=seed, whitelist=whitelist, att_chance=att_chance,extended_mode=extended_mode) #if extended mode = 3, dann length (e.g. 3) * vec (8) * modes (2) = 48
    final = {'epochs': [50], 'negative_sampling': [10], #'vector_dimensions': [], will be added after the fact
             'degree': [2], 'alpha': [0.008], 'window': [2], 'min_count':[4], 'columns': ['dummy']
             #'name': hyperparams['name']+'_final'
             }

    hyperparams = pd.DataFrame.from_dict(final)  # this should consider duplication for extended mode
    hyperparams = hyperparams.loc[hyperparams.index.repeat(len(hyperparams_name))].reset_index()
    hyperparams['name'] = hyperparams_name['name'] + '_final' #should be length * 8 after this step
    hyperparams['extended_mode'] = hyperparams_name['extended_mode']
    hyperparams = hyperparams.sort_values('extended_mode') #sort to ensure that not 2* vec dim 2 with same extended mode etc.
    multiplyer = int(len(hyperparams_name) / len(vector_dimensions)) # this should be always int. precisely length (i.e. how many times each combination is repeated) * number of extended modes (1 or 2)
    hyperparams['vector_dimensions'] = vector_dimensions * multiplyer #vector_dimensions * multiplyer
    hyperparams.to_sql('hyperparams_final', con=engine, if_exists='append', index=False)
    return hyperparams
...
def embed(hyperparams, graphset, engine, engine_info, name='embedding_', graphsetname='default', bpic=None, default_prefix =''):
    ...
    embedding.to_sql(table_name, con=engine, if_exists='replace', index=False)
    lookup = pd.DataFrame({'scenario': scenario['name'], 'embedding': table_name}, columns=['scenario','embedding'], index = [0])
    lookup.to_sql('hyperparams_to_embedding_final', con=engine, if_exists='append', index=False)
    ...
    log_df.to_sql('log_final',engine_info,if_exists='append')
...
def main(whitelist=None, att_chance=None):
    ...
    length = 3  # number of times each scenarios / hyperparam
    ...
    for bpic in bpics:
        if bpic == 15:
            ...
            engine_embedding = create_engine('sqlite:///../Data/Database/embedding_bpic15_final.db', echo=False)
            ...
            engine_embedding = create_engine('sqlite:///../Data/Database/embedding_bpic19_final.db', echo=False)
        ...
        hyperparams['columns'] = run_cols_list * length * 2 * 9 # * 2 bc extended mode is additionally to all hyperparam combinaitions, *8 bc each vector is run
        #hyperparams['columns'] = run_cols_list * length * 2 * 1  # * 2 special for vec dim 3 only
        ...
        table_name = 'nmi_' + str(bpic) + '_final'
