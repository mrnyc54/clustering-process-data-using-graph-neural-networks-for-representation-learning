...
def log_final_metrics(model, test_loader, device, engine, name_model, name_hyper, dataset_name , actual_epoch, share_ano_train, share_train, share_ano_val, share_val, share_ano_test, share_test):
    ...
    tablename = 'result_final_' + str(name_model)
    df_tmp.to_sql(tablename, engine,if_exists='append')
    return test_score_f1, test_score_roc


#def model_runner(model_name, hyperparam_name, dataset_name, num_features, device, hidden_gin, ratio_topk, learning_rate, train_loader, val_loader, test_loader, patience, min_diff):
def model_runner(scenario, dataset_name, graphset, device, train_loader, val_loader, test_loader, patience, min_diff):
    ...
    # GCN Network
    print(f'Start Training {model_name} Model. Training {training_cntr} of {total_trainings}.')
    get_model = {
        'GCN': GCNet(graphset.num_features).to(device),
        'GIN': GINConvNet(graphset.num_features, hidden_gin).to(device),
        'TopK': TopKNet(graphset.num_features, ratio_topk).to(device)
    }
    weight = torch.div(torch.sum(graphset.data.y == 0),
                       torch.sum(graphset.data.y == 1))  # get ratio of true to false cases
    model =  get_model.get(model_name)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    crit = torch.nn.BCEWithLogitsLoss(pos_weight=weight)
    best_model, epoch = training_early_stop(model, epochs, train_loader, val_loader, device, optimizer,
                                            crit, patience, min_diff)
    missing, unexpected = model.load_state_dict(best_model)
    test_score_f1, test_score_roc = log_final_metrics(model, test_loader, device, engine, model_name, hyperparam_name,
                                                      dataset_name, epoch, percent_true_train, share_train,
                                                      percent_true_val, share_val, percent_true_test, share_test)
    print(f'Done Training {model_name} Model. Eval: F1: {test_score_f1:.5f}, AUC:{test_score_roc:.5f}')
...
engine = create_engine('sqlite:///../../Data/Database/results_final.db', echo=False)
resultname = 'result'
...
hyperparams = {
    'model': ['GCN', 'GIN', 'TopK'],
    'epochs': [100, 100, 100],
    'alpha': [0.024, 0.01, 0.001],
    'batch_size': [256, 512, 64],
    'hidden_gin': [32, 256, 256],
    'ratio_topk': [0.1, 0.8, 0.6]
}
hyperparams = pd.DataFrame(hyperparams)
hyperparams_names = gen_df(len(hyperparams))
hyperparams['name'] = hyperparams_names['name']
hyperparams.to_sql('hyperparams_final', engine, if_exists='append')
length = len(hyperparams) #No. of hyperparameter configurations each Dataset gets run at (*3 for each Model)
print('Hyperparameters of length {} created and saved!'.format(length))

# specify base share per dataset split
share_train = 0.5
share_val = 0.2
share_test = 0.3
...
length = 10
hyperparams = hyperparams.loc[hyperparams.index.repeat(length)].reset_index()
print('test hyperparams: \n', hyperparams)
...
total_trainings = len(hyperparams) * len(graph_names_all)
training_cntr = 1
    ...
    for graph_name in graph_names_all:
        ...
        if 'event' in graph_name:
            ...
            if '_0_0' in dataset_name:
                ...
                train_loader, val_loader, test_loader = split_dataset(graphset, backup_graphset, batch_size, share_train, share_val, ratio_test=share_test, test_anom=False)
            else:
        ...
        #run models
        model_runner(scenario, dataset_name, graphset, device, train_loader, val_loader, test_loader, patience, min_diff)
        training_cntr += 1
...
run_log.to_sql('runs_final', engine, if_exists='append')
...
run_map.to_sql('run_hyperparam_final', engine, if_exists='append')
